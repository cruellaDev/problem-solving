...
def solution(new_id):
    ...
    # 1 - 대문자 -> 소문자
    new_id = new_id.lower()
    # 2 - 알파벳 소문자, 숫자, 빼기(-), 밑줄(_), 마침표(.) 만 가능
    new_id = re.sub(pattern, '', new_id)
    # 3 - 마침표 중복 제거
    while '..' in new_id:
        new_id = new_id.replace(period * 2, period)
    # 4 - 처음과 끝 마침표 제거
    if new_id and new_id[0] == period:
        new_id = new_id[1:]
    if new_id and new_id[-1] == period:
        new_id = new_id[:-1]
    # 5 - 빈 문자열은 'a'대입
    if new_id == '':
        new_id = 'a' * min_len
    # 6 - 길이 16자일 시 첫 15개 문자만 남김
    if len(new_id) > max_len:
        new_id = new_id[:max_len]
    # 제거 후 마침표(.)가 new_id의 끝에 위치한다면 끝에 위치한 마침표(.) 문자 제거
    if new_id[-1] == period:
        new_id = new_id[:-1]
    # 7 - 길이 2자 이하 시 new_id의 마지막 문자를 new_id의 길이가 3이 될 때까지 붙임
    if len(new_id) < min_len:
        while len(new_id) < min_len:
            new_id += new_id[-1]
    return new_id
